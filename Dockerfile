...
ARG BRANCH=master
FROM metwork/mfxxx-centos7-buildimage:${BRANCH} as yum_cache
ARG BRANCH
...
FROM metwork/mfxxx-centos7-buildimage:${BRANCH}
ARG BRANCH
COPY --from=yum_cache /etc/yum.repos.d/metwork.repo /etc/yum.repos.d/
COPY --from=yum_cache /tmp/yum_cache .
RUN yum clean all
RUN yum -y install metwork-mfbase-full initscripts cronie vim
